...
Consulta que agrupe la cantidad actual de contagios por país.*/
	...
	SELECT CASOS.ID_PAIS, PAIS.LOCATIONN , CAST(CASOS.TOTAL_CASES as INT)TOTAL_CASOS , CASOS.DATEE FROM CASOS, PAIS
	WHERE CASOS.ID_PAIS = PAIS.ID_PAIS AND CASOS.DATEE= DATEADD(day,-2,CONVERT(date,GETDATE()));
...
and casos.DATEE between DATEADD(day,-25,CONVERT(date,GETDATE())) and DATEADD(day,-2,CONVERT(date,GETDATE()))
